        ...
        if($result  = mysqli_query($link,$query)){
            ...
            while($rowa=mysqli_fetch_array($result)){
                    ...
                    if ($resulti = mysqli_query($link, $query2)) {
                        ...
                        if($rowi['idstatus']<>2) {
                            $query58 = "SELECT `id` FROM `mangasfav` WHERE(`idmanga`='" . mysqli_real_escape_string($link, $rowa['id']) . "' AND `iduser`='" . mysqli_real_escape_string($link, $_SESSION['id']) . "')";
                            $result58 = mysqli_query($link, $query58);
